...
async function getLeagueData() {
  return await readJson(LEAGUE_DATA_FILE, {
    ...
    leagueSettingsHtml: "<h2>League Settings</h2><ul><li>Scoring: Standard</li><li>Transactions counted from <b>Thu 12:00 AM PT â†’ Wed 11:59 PM PT</b>; first two are free, then $5 each.</li></ul>",
    tradeBlock: [],
    rosters: {},
...
async function fetchSeasonMovesAllSources({ leagueId, seasonId, req, maxSp=25, onProgress }){
  ...
  for (let sp=1; sp<=maxSp; sp++){
    onProgress?.(sp, maxSp, "Reading ESPN activityâ€¦");
    try { 
...
async function fetchRosterSeries({ leagueId, seasonId, req, maxSp=25, onProgress }){
  ...
  for (let sp=1; sp<=maxSp; sp++){
    onProgress?.(sp, maxSp, "Building roster timelineâ€¦");
    let byTeam = {};
...
app.post("/api/report/update", async (req, res) => {
  ...
  try {
    setProgress(jobId, 5, "Fetching teamsâ€¦");
    ...
    setProgress(jobId, 92, "Computing official totalsâ€¦");
