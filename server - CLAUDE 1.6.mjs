...
async function buildPlayerMap({ leagueId, seasonId, req, ids, maxSp=25, onProgress }){
  ...
  // First try to get names from current roster data
  for (let sp=1; sp<=maxSp; sp++){
  ...
  // If we still have missing names, try to get them from transaction data
  if (need.size > 0) {
    console.log(`[DEBUG] Still missing ${need.size} player names, trying transaction data...`);
    for (let sp=1; sp<=maxSp && need.size > 0; sp++) {
      try {
        const r = await espnFetch({ leagueId, seasonId, view:"mTransactions2", scoringPeriodId: sp, req, requireCookie:true });
        const transactions = r?.transactions || [];
        
        for (const t of transactions) {
          const items = t.items || [];
          for (const item of items) {
            const p = item.playerPoolEntry?.player;
            const pid = p?.id;
            if (pid && need.has(pid)) {
              map[pid] = p.fullName || p.name || `Player ${pid}`;
              need.delete(pid);
            }
          }
        }
  ...
  // Final fallback: use generic names for any remaining IDs
  for (const pid of need) {
    map[pid] = `Player ${pid}`;
    console.log(`[DEBUG] Could not resolve name for player ID ${pid}`);
  }
