...
async function buildOfficialReport({ leagueId, seasonId, req, logger }) {
  logger?.info(`Starting official report build`, { leagueId, seasonId });
  ...
  logger?.info(`After initial deduplication`, { 
    originalCount: all.length,
    dedupedCount: deduped.length,
    removedByDedup: all.length - deduped.length
  });

  // ===== ENHANCED VERIFICATION WITH DETAILED LOGGING =====
  const verified = [];
  let verificationStats = {
    adds: { total: 0, verified: 0, reasons: {} },
    drops: { total: 0, verified: 0, reasons: {} }
  };

  for (const r of deduped) {
    if (r.action === "ADD") {
      verificationStats.adds.total++;
      
      if (!r.playerId) {
        verified.push(r);
        verificationStats.adds.verified++;
        verificationStats.adds.reasons.noPlayerId = (verificationStats.adds.reasons.noPlayerId || 0) + 1;
        continue;
      }
      ...
      const appears = isOnRoster(series, sp, tid, r.playerId) ||
      ...
      if (appears) {
        verified.push(r);
        verificationStats.adds.verified++;
        verificationStats.adds.reasons.rosterVerified = (verificationStats.adds.reasons.rosterVerified || 0) + 1;
      } else {
        verificationStats.adds.reasons.notOnRoster = (verificationStats.adds.reasons.notOnRoster || 0) + 1;
        logger?.debug(`ADD filtered out - not on roster`, {
          player: r.playerName || r.playerId,
          team: r.team,
          method: r.method,
          sp,
          date: r.date
      ...
      verificationStats.drops.total++;
      
      if (!r.playerId) {
        verificationStats.drops.reasons.noPlayerId = (verificationStats.drops.reasons.noPlayerId || 0) + 1;
        continue;
      }
      ...
      const wasBefore = isOnRoster(series, Math.max(1, sp-1), tid, r.playerId) ||
                       isOnRoster(series, sp, tid, r.playerId);
      const appearsLater = isOnRoster(series, sp+1, tid, r.playerId) ||
                          isOnRoster(series, sp+2, tid, r.playerId);

      if (wasBefore && !appearsLater) {
        verified.push(r);
        verificationStats.drops.verified++;
        verificationStats.drops.reasons.executed = (verificationStats.drops.reasons.executed || 0) + 1;
      } else {
        verificationStats.drops.reasons.notExecuted = (verificationStats.drops.reasons.notExecuted || 0) + 1;
        logger?.debug(`DROP filtered out - not executed`, {
          player: r.playerName || r.playerId,
          team: r.team,
          wasBefore,
          appearsLater,
  ...
  logger?.info(`Roster verification complete`, verificationStats);
  ...
  for (const r of verified) {
    if (!r.player && r.playerId) {
      r.player = pmap[r.playerId] || `#${r.playerId}`;
    }
  }

  // Process into final moves
  ...
  logger?.info(`Final deduplication complete`, {
    beforeFinalDedup: verified.length,
    afterFinalDedup: rawMoves.length,
    finalDedupRemoved: finalDedupCount
  });
  ...
  const finalStats = {
    totalFinalTransactions: rawMoves.length,
    totalFinalAdds: rawMoves.filter(r => r.action === "ADD").length,
    totalFinalDrops: rawMoves.filter(r => r.action === "DROP").length,
    transactionsByWeek: Object.fromEntries([...perWeek.entries()].map(([w, m]) => [
      w, [...m.entries()].reduce((sum, [team, count]) => sum + count, 0)
    ]))
  };

  logger?.info(`Report build complete`, finalStats);
  ...
  return { 
    ...
    _diagnostics: {
      verificationStats,
      finalStats,
      processingLog: logger.getSummary()
    }
  };
}
...
app.post("/api/report/update", async (req, res) => {
  const logger = new ProcessLogger();
  logger.info("[REPORT/UPDATE] Starting report update", req.body);
  ...
  try {
    ...
    await writeJson(`diagnostics_${seasonId}.json`, logger.getFullLog());
    ...
    logger.info("[REPORT/UPDATE] Complete", summary);
    
    res.json({ 
      ok: true, 
      weeks: (report?.weekRows || []).length,
      diagnostics: summary
    });
  } catch (err) {
    logger.error("[REPORT/UPDATE] Failed", err);
