...
const validTransactions = deduped.filter(r => r.method !== "CANCEL");

// For dues calculation, exclude draft picks
const billableAdds = validTransactions.filter(r => 
  r.action === "ADD" && r.method !== "DRAFT"
);

// All drops count for context (but don't affect dues)
const drops = validTransactions.filter(r => r.action === "DROP");
...
console.log(`[DEBUG] Excluded CANCEL transactions, kept PROCESS and EXECUTE`);
 ...
 const needIds = [...new Set([...billableAdds, ...drops].map(r => r.player ? null : r.playerId).filter(Boolean))];
