...
async function buildOfficialReport({ leagueId, seasonId, req, logger }){
  ...
  // Build per-week per-team "entries" with dues ($5 after first 2 adds) and season totals.
  const perWeekAdds = new Map(); // week -> Map(teamName -> count)
  ...
  const totals = new Map(); // teamName -> {adds, owes}
  const weekRows = [];      // [{week, range, entries:[{name,count,owes}]}]
  for (const [w, m] of [...perWeekAdds.entries()].sort((a,b)=> a[0]-b[0])) {
    ...
    weekRows.push({ week:w, range: rangeByWeek.get(w) || "", entries });
  }
  ...
  const reportOut = {
    ...
    // New fields
    weeks: weekRows,           // alias for weekRows
    rows: rawMoves,            // alias for rawMoves
    totalAdds: rawMoves.filter(r=>r.action==="ADD").length,
    totalDrops: rawMoves.filter(r=>r.action==="DROP").length,
    // Legacy fields (for existing UI screens like Dues)
...
app.post("/api/report/update", async (req, res) => {
  try {
    ...
    if (seasonId) { await writeJson(`report_${seasonId}.json`, report); }
    await fs.mkdir(DATA_DIR, { recursive: true });
...
app.get("/api/report", async (req, res) => {
  ...
  let data = null;
  if (seasonId) {
    data = await readJson(`report_${seasonId}.json`, null);
  }
