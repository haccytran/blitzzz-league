...
app.get("/api/leagues/:leagueId/strength-of-schedule/:seasonId", async (req, res) => {
  try {
    ...
    // Use the same helper as other endpoints
    const data = await callPythonService('/strength-of-schedule', {
      leagueId: espnLeagueId,
      seasonId: seasonId,
...
if (rec.method === "PROCESS" && rec.add) {
  ...
  if (onRoster) {
    ...
    if (rec.bidAmount !== null && rec.bidAmount !== undefined) {
      console.log(`[DEBUG] Waiver winner (bid $${rec.bidAmount}): Team ${teamId} gets player ${playerId}`);
    }
    continue;
  }
