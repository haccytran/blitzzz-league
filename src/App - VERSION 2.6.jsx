...
/* ---- Editor pieces ---- */
function AnnouncementEditor({ html, setHtml, onPost, disabled }) {
  const ref = useRef(null);

  // keep focus/selection so toggles work
  ...
  const handle = (cmd, val) => (e) => {
    e.preventDefault();          // use onMouseDown + preventDefault to keep selection
    exec(cmd, val);
  };
  ...
  return (
    <div className="card" style={{ padding: 16, background: "#f8fafc" }}>
      ...
      <div style={{ display:"flex", flexWrap:"wrap", gap:8, marginBottom:8 }}>
        <button className="btn" style={btnSec} onMouseDown={handle("bold")}><b>B</b></button>
        <button className="btn" style={btnSec} onMouseDown={handle("italic")}><i>I</i></button>
        <button className="btn" style={btnSec} onMouseDown={handle("underline")}><u>U</u></button>
        <button className="btn" style={btnSec} onMouseDown={handle("strikeThrough")}><s>S</s></button>

        <span style={{ width:8 }} />

        <button className="btn" style={btnSec} onMouseDown={handle("insertUnorderedList")}>• List</button>
        <button className="btn" style={btnSec} onMouseDown={handle("insertOrderedList")}>1. List</button>
        ...
        <button className="btn" style={btnSec} onMouseDown={handle("removeFormat")}>Clear</button>
      </div>
  ...
  onClick={() => {
    ...
    if (!html || html === "<br>") { alert("Type something first"); return; }
    onPost(html);
    if (ref.current) {
...
const [draftHtml, setDraftHtml] = useState(data.settingsHtml || "");
useEffect(() => { setDraftHtml(data.settingsHtml || ""); }, [data.settingsHtml]);
...
<div style={{ textAlign: "right", marginTop: 8 }}>
  <button
    ...
    style={btnPri}
    disabled={!isAdmin}
    onClick={() => setData(d => ({ ...d, settingsHtml: draftHtml }))}
  >
    Save
  </button>
</div>
...
function RichEditor({ html, setHtml, readOnly }) {
  ...
  const ref = useRef(null);
  const lastTyped = useRef(null); // tracks content that came from typing

  // If parent changes `html`, mirror to local
  useEffect(() => {
    setLocal(html || "");
  }, [html]);

  // Mount: put the initial HTML into the div once
  useEffect(() => {
  ...
  // When `local` changes from the OUTSIDE (not from typing),
  // push it into the DOM exactly once so caret doesn't jump.
  useEffect(() => {
  ...
  if (readOnly) {
    return (
      <div className="card" style={{ padding: 16 }}>
        <div
          className="prose"
          dangerouslySetInnerHTML={{ __html: local || "<p>No settings yet.</p>" }}
        />
      </div>
    );
  }
  ...
  const exec = (cmd, val = null) => {
    ...
    lastTyped.current = htmlNow;      // mark change as "typed"
    setLocal(htmlNow);
  };
  ...
  return (
    <div className="card" style={{ padding: 16, background: "#f8fafc" }}>
      ...
      <div
        ...
        /* IMPORTANT: we removed dangerouslySetInnerHTML here */
      />
