...
function LeagueHub(){
  ...
  // default tab from the URL (fallback = activity)
  const initialTabFromHash = () => {
    const h = (window.location.hash || "").replace("#","").trim();
    return VALID_TABS.includes(h) ? h : "activity";
  };

  // keep this state line where you already have it (or add it if missing):
  ...
  const defaultData = {
    ...
    members: [],
    waivers: [],
    buyins: {},            // NEW: per-season buy-in tracking
    leagueSettingsHtml:
  ...
  return (
    <a
      ...
      style={{
        ...
        margin: "6px 0",
        color: "#e2e8f0",
        fontSize: 14,                // back to old size
        // no background / border here — CSS will handle
      }}
    >
      {label}
    </a>
...
function BuyInTracker({ isAdmin, members, seasonYear, data, setData }) {
  ...
  const copyZelle = async () => {
    ...
    try { await navigator.clipboard.writeText(email); alert("Zelle email copied to clipboard!"); }
    catch { alert("Could not copy. Long-press / right-click to copy instead: " + email); }
  };
  ...
  return (
    <div className="card" style={{ padding:16, marginTop:16 }}>
      ...
      {members.length > 0 && (
        <div className="grid" style={{gridTemplateColumns:"320px 260px", gap:16}}>
          {/* LEFT: checklist */}
          <div className="card" style={{ padding:12, background:"#f8fafc" }}>
            <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8}}>
              <strong>League Owners</strong>
              {isAdmin && (
                <div style={{display:"flex", gap:8}}>
          ...
          <div className="card buyin-pay" style={{ padding:12 }}>
            ...
            {(cur.venmoQR || cur.venmoLink || cur.zelleEmail) && (
              <div style={{marginTop:8}}>
                ...
                >
                  {cur.venmoQR && (
                    <img src={cur.venmoQR} alt="Venmo QR"/>
                  )}
                </a>
              </div>
            )}
      ...
      {report && (
        <div className="dues-grid dues-tight">
          ...
          <div className="dues-left">
            <div className="card" style={{padding:12}}>
              <h3 style={{marginTop:0}}>Season to Date</h3>
